...
for index, row in df.iterrows():
    post = Post(row['user_id'], row['creation time'], row['text'], row['image_url'], row['expiration in hour'])
...
for post in loaded_posts:
    posts_collection.insert_one(post.as_dictionary())
...
total_posts_inserted = db.command("collstats", "posts")['count']
